...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="make-schema-toc">
        ...
        <xsl:for-each-group select="$items[sf:path-depth(re:spfe-configuration-reference-entry/re:doc-xpath)=$level]" group-by="re:spfe-configuration-reference-entry/re:doc-xpath">
            <!-- FIXME: the node id should probably be read from the ss:topic/@local-name rather than recalculated like this -->
            <node id="{translate(re:spfe-configuration-reference-entry/re:xpath, '/:', '__')}"
                name="{re:spfe-configuration-reference-entry/re:name}">
                <xsl:call-template name="make-schema-toc">
                    <xsl:with-param name="level" select="$level+1"/>
                    <xsl:with-param name="items" select="$items[starts-with(re:spfe-configuration-reference-entry/re:xpath, concat(current()/re:spfe-configuration-reference-entry/re:xpath, '/'))]"/>
                </xsl:call-template>
            </node>
        </xsl:for-each-group>
    </xsl:template> 

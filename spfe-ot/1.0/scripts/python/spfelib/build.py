...
def build_topic_sets(config):
    topic_set_id_list = []
    for topic_set in config.iter_config_subset('content-set/topic-set'):
        topic_set_id_list.append(config.setting('topic-set-id', topic_set))
    object_set_id_list = []
    for object_set in config.iter_config_subset('content-set/object-set'):
        object_set_id_list.append(config.setting('object-set-id', object_set))
...
def _build_synthesis_stage(config, *, topic_set_id=None, object_set_id=None):
    ...
    ts_config = config.config_subset('content-set/topic-set[topic-set-id="{tsid}"]'.format(tsid=topic_set_id)

    ) if topic_set_id is not None else config.config_subset(
    ...
    resolve_output_dir = posixpath.join(posixpath.dirname(config.build_scripts[set_id][('resolve', None)]),'out') if topic_set_id is not None else posixpath.join(config.content_set_build_dir, 'objects', set_id)
...
def _build_extract_step(config, set_id, set_type, script, output_dir, source_files):
    ...
    util.run_XSLT2(script=script, env=config.spfe_env, infile=infile, outfile=outfile, initial_template='main', **parameters)
...
def _build_presentation_stage(config, topic_set_id):
    ...
    for presentation_type in [item[1] for item in config.build_scripts[topic_set_id] if item[0] == 'present']:
        present_output_dir = posixpath.join(posixpath.dirname(config.build_scripts[topic_set_id][('present', presentation_type)]),'out')
        _build_present_step(config=config,
                            ...
                            synthesis_files= [x.replace('\\', '/') for x in glob(posixpath.join(posixpath.dirname(config.build_scripts[topic_set_id][('resolve', None)]),'out')+'/*')],
                            toc_files=glob(posixpath.join(config.content_set_build_dir, 'tocs')+'/*'),
                            link_catalog_files=glob(posixpath.join(config.content_set_build_dir, 'link-catalogs')+'/*'),
                            object_files=[x.replace('\\', '/') for x in glob(posixpath.join(config.content_set_build_dir, 'objects')+'/*/*')])
...
def _build_formatting_stage(config, topic_set_id):
    ...
    for format_type in [item[1] for item in config.build_scripts[topic_set_id] if item[0] == 'format']:
        ...
        presentation_type = config.setting('content-set/output-formats/output-format[name="{ft}"]/presentation-type'.format(
             ft=format_type))
        ...
        try:
            presentation_files = [x.replace('\\', '/') for x in glob(posixpath.join(posixpath.dirname(config.build_scripts[topic_set_id][('present', presentation_type)]),'out')+'/*')]
        except KeyError:

...
<!-- processing directives -->
	...
	<xsl:variable name="synonyms-text" xml:base="synonyms/">
		<xsl:for-each select="tokenize(translate($synonyms-files-names,'\','/'), $config/config:dir-separator)"><xsl:value-of select="unparsed-text(.)"/></xsl:for-each>
	</xsl:variable>
...
-->
	...
	<xsl:template match="ss:synthesis">
		<link-catalog topic-set-id="{$config/config:topic-set-id}" output-directory="{$config/config:deployment/config:output-path}" product="{$config/config:publication-info/config:product}" release="{$config/config:publication-info/config:release}" time-stamp="{current-dateTime()}">
			<xsl:apply-templates/>
			<!-- hack in the title page until we get titles worked out -->
			<xsl:call-template name="title-page-index"/>
		</link-catalog>
	</xsl:template>
	
	<xsl:template match="ss:topic">
		<xsl:variable name="name" select="@local-name"/>
		<page name="{@local-name}" title="{@title}" file="{@local-name}.html" topic-type="{if (@virtual-type) then @virtual-type else @type}">
			<xsl:choose>
				<xsl:when test="@scope">
			...
			<target type="topic">
				...
				<key><xsl:value-of select="@full-name"/></key>
			</target>
        	...
        	<xsl:call-template name="warning">
        		<xsl:with-param name="message">only one key allowed per reference - please split up your keys: key 1 is "<xsl:value-of select="*:term[1]"/>", key 2 is "<xsl:value-of select="*:term[2]"/>"
        		</xsl:with-param>
        	</xsl:call-template>
        </xsl:if>
            </original-key>
					<xsl:variable name="references">
						<xsl:for-each-group select="*:term[normalize-space(.) ne '']" group-by=".">
							...
							<xsl:for-each
							 select="$synonyms/synonyms/synonym[word=current()]/word">
								<xsl:call-template name="construct-index-key">
									<xsl:with-param name="key" select="."/>
								</xsl:call-template>
							</xsl:for-each>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
								<key-set>
									...
									<xsl:for-each select="$synonyms/synonyms/synonym[word=regex-group(1)]/word[. ne regex-group(1)]">
	...
	<xsl:template name="title-page-index">
		<page name="{$config/config:topic-set-id}" title="{$config/config:publication-info/config:title}" file="title.html" topic-type="title-page">
			<xsl:if test="$config/config:default-topic-scope">
				<xsl:attribute name="scope" select="$config/config:default-topic-scope"/>
			</xsl:if>
			<xsl:if test="$config/config:optional-product">
				<xsl:attribute name="optional-product" select="$config/config:optional-product"/>
			...
			<!-- topic references -->
			<target type="topic-set">
				<key><xsl:value-of select="$config/config:topic-set-id"/></key>
			</target>

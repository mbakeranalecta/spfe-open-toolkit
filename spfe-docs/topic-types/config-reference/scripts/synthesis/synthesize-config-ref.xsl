...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="output-directory"
		select="concat($config/config:doc-set-build, '/topic-sets/', $topic-set-id, '/synthesis/out')"/>
...
-->
	...
	<xsl:template match="schema-element">
		...
		<xsl:if test="($current-doctype = $doctype) or not($doctype)">
			...
			<ss:topic type="http://spfeopentoolkit.org/spfe-docs/topic-types/config-reference"
				...
				local-name="{translate(xpath, '/:', '__')}" topic-type-alias="{$topic-type-alias}"
				title="{name}"
				...
				<ss:index>
					...
					<xsl:for-each
						...
						<ss:entry>
							<ss:type>config-setting</ss:type>
							<ss:namespace>http://spfeopentoolkit.org/spfe-ot/1.0/schemas/spfe-config</ss:namespace>
							<ss:term><xsl:value-of select="$xpath"/>/@<xsl:value-of select="name"
								/></ss:term>
							<ss:anchor>
					...
					<xsl:if
						test="normalize-space($source[normalize-space(ed:xpath)=$xpath][1]/ed:build-property)">
						<ss:entry>
							<ss:type>spfe-build-property</ss:type>
							<ss:namespace>http://spfeopentoolkit.org/spfe-ot/1.0/build</ss:namespace>
							<ss:term>
								<xsl:value-of
									select="normalize-space($source[normalize-space(ed:xpath)=$xpath][1]/ed:build-property)"
								/>
							</ss:term>
						</ss:entry>
					</xsl:if>
	...
	<xsl:template
		match="ed:restrictions | ed:restriction | ed:values | ed:default | ed:value | ed:name">
...
-->
	...
	<xsl:template match="ed:config-setting" priority="1">
		<xsl:variable name="context-element"
			select="ancestor::ed:config-setting-description/normalize-space(ed:xpath)"/>

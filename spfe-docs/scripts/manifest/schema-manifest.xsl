...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:import href="http://spfeopentoolkit.org/spfe-ot/plugins/eppo-simple/scripts/presentation/common/present-manifest.xsl"/>
    
    <xsl:template match="topics-of-type[@type='http://spfeopentoolkit.org/spfe-docs/schemas/authoring/spfe-configuration-reference-entry']" mode="manifest">
        
        <xsl:for-each-group select="//re:spfe-configuration-reference-entry" group-by="re:schema-element/re:doctype">
            <xsl:sort select="current-grouping-key()"/>
            <xsl:call-template name="make-schema-manifest">
    ...
    <xsl:template name="make-schema-manifest">
